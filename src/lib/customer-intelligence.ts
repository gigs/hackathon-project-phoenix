...
export function currentLines(data: CustomerData): number | null {
  const latestSeriesValue = [...data.arrData]
    .reverse()
    .find((point) => point.linesActual != null)?.linesActual;
  if (latestSeriesValue != null) return latestSeriesValue;
  const total = data.deals.reduce((sum, deal) => sum + (deal.activations ?? 0), 0);
  return total > 0 ? total : null;
}

export function currentArr(data: CustomerData): number | null {
  const latestSeriesValue = [...data.arrData].reverse().find((point) => point.actual != null)?.actual;
  return latestSeriesValue ?? sumArr(data);
}
